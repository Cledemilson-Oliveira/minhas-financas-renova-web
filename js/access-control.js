...
let entitlement=null,user=null;
...
function isLimited(){return entitlement?.mode==='limited'}
function can(feature){const f=entitlement?.features||{};return f.all===true||f[feature]===true}
...
async function refresh(){if(!user)return;const {data,error}=await supabase.rpc('get_my_plan_access');if(error)return;entitlement=data||null;renderNotice();syncButtons()}
