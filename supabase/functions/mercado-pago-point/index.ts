...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(action==="get_pending_order"){
      ...
      const {data:terminal}=await admin.from("payment_terminals").select("id").eq("id",localTerminalId).eq("user_id",user.id).eq("provider","mercado_pago").maybeSingle();if(!terminal?.id)return reply({error:"terminal_not_found"},404);
      const {data:stored,error:storedError}=await admin.from("payment_orders").select("id,provider_order_id,status,status_detail,transaction_id,created_at").eq("user_id",user.id).eq("provider","mercado_pago").eq("terminal_id",localTerminalId).in("status",["created","pending","at_terminal"]).order("created_at",{ascending:false}).limit(1).maybeSingle();
      if(storedError)return reply({error:"pending_order_lookup_failed",message:storedError.message},500);if(!stored?.id||!stored.provider_order_id)return reply({ok:true,order:null});
      const {res,data}=await mp(`/v1/orders/${encodeURIComponent(stored.provider_order_id)}`,token);
      if(!res.ok)return reply({ok:true,order:stored,provider_order:null});
      const status=normalizeStatus(data?.status);const {data:updated}=await admin.from("payment_orders").update({status,status_detail:String(data?.status_detail||data?.status||status),updated_at:new Date().toISOString(),provider_data:{external_terminal_id:data?.config?.point?.terminal_id||null,order_status:String(data?.status||status),payment_status:data?.transactions?.payments?.[0]?.status||null,provider_order:data}}).eq("id",stored.id).select("id,provider_order_id,status,status_detail,transaction_id,created_at").single();
      return reply({ok:true,order:updated||{...stored,status,status_detail:String(data?.status_detail||data?.status||status)},provider_order:data});
    }
