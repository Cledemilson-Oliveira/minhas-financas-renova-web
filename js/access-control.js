...
let entitlement=null,user=null,accessRow=null,accessReady=false;
...
function isOwner(){const status=String(accessRow?.status||'').toLowerCase();return entitlement?.mode==='owner'||(accessRow?.role==='dono'&&(status==='ativo'||status==='active'))}
function isLimited(){return accessReady&&!isOwner()&&entitlement?.mode==='limited'}
function can(feature){if(isOwner())return true;if(!accessReady)return true;const f=entitlement?.features||{};return f.all===true||f[feature]===true}
...
function mutationLocked(){return accessReady&&isLimited()}

function guardClick(e){if(!accessReady||isOwner()||entitlement?.mode==='owner')return;const pageBtn=e.target.closest('[data-page]');if(pageBtn){const page=pageBtn.dataset.page;if(page&&corePages[page]&&!pageAllowed(page)){e.preventDefault();e.stopImmediatePropagation();toast('Seu período ativo terminou. Escolha um plano para continuar usando este módulo.','error');goSubscription();return}}
...
async function refresh(){if(!user)return;accessReady=false;const [accessResp,entitlementResp]=await Promise.all([supabase.from('user_access').select('role,status').eq('user_id',user.id).maybeSingle(),supabase.rpc('get_my_plan_access')]);if(!accessResp.error)accessRow=accessResp.data||accessRow;if(!entitlementResp.error)entitlement=entitlementResp.data||entitlement;accessReady=true;renderNotice();syncButtons()}

ensureStyles();document.addEventListener('click',guardClick,true);window.addEventListener('renova:entitlement',e=>{entitlement=e.detail||entitlement;if(entitlement?.mode==='owner')accessReady=true;renderNotice();syncButtons()});const {data:{session}}=await supabase.auth.getSession();user=session?.user||null;if(user)await refresh();supabase.auth.onAuthStateChange((_e,s)=>{user=s?.user||null;accessReady=false;if(user)setTimeout(refresh,0);else{accessRow=null;entitlement=null;renderNotice();syncButtons()}});
